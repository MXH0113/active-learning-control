...
for k = 2:N-1
    ...
    u_16(k) = (input_ref(k+1)-theta_16(1)*w_f'*h_f(:,k)-theta_16(3))/(theta_16(2)*w_g'*h_g(:,k));
    u_17(k) = (input_ref(k+1)-theta_17(1)*w_f'*h_f(:,k)-theta_17(3))/(theta_17(2)*w_g'*h_g(:,k));
    u_18(k) = (input_ref(k+1)-theta_18(1)*w_f'*h_f(:,k)-theta_18(3))/(theta_18(2)*w_g'*h_g(:,k));
    u_19(k) = (input_ref(k+1)-theta_19(1)*w_f'*h_f(:,k)-theta_19(3))/(theta_19(2)*w_g'*h_g(:,k));
    u_20(k) = (input_ref(k+1)-theta_20(1)*w_f'*h_f(:,k)-theta_20(3))/(theta_20(2)*w_g'*h_g(:,k));
    u_21(k) = (input_ref(k+1)-theta_21(1)*w_f'*h_f(:,k)-theta_21(3))/(theta_21(2)*w_g'*h_g(:,k));

    u_opt(k) = (input_ref(k+1)-alpha_real(k)*w_f'*h_f(:,k)-gamma_real(k))/(beta_real(k)*w_g'*h_g(:,k));

    u_q(k) = q_1*u_1(k)+q_2*u_2(k)+q_3*u_3(k)+q_4*u_4(k)+q_5*u_5(k)+q_6*u_6(k)+q_7*u_7(k)+q_8*u_8(k)+q_9*u_9(k)+q_10*u_10(k)...
         +q_11*u_11(k)+q_12*u_12(k)+q_13*u_13(k)+q_14*u_14(k)+q_15*u_15(k)+q_16*u_16(k)+q_17*u_17(k)+q_18*u_18(k)+q_19*u_19(k)+q_20*u_20(k)+q_21*u_21(k);
